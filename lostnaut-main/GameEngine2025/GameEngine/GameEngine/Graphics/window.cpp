...
void Window::setKey(int key, bool ok) { this->keys[key] = ok; }
...
void Window::setMousePos(double xpos, double ypos) {
  ...
  mouseDeltaY =
      lastMouseY - ypos; // Reversed: y-coordinates go from bottom to top
...
bool Window::isPressed(int key) { return keys[key]; }

// Handling mouse buttons pressed
bool Window::isMousePressed(int button) { return mouseButtons[button]; }
